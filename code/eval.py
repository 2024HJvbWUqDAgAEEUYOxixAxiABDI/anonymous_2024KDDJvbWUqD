...
@torch.no_grad()
def evaluate(model, dataset, split_idx, eval_func, criterion, args, degrees, threshold, result=None):
    ...
    top20_indices = split_idx['test'][degrees_in_test > threshold]
    bottom80_indices = split_idx['test'][degrees_in_test < threshold]
    top20_acc = eval_func(
        dataset.label[top20_indices], out[top20_indices]
    )
    bottom80_acc = eval_func(
        dataset.label[bottom80_indices], out[bottom80_indices]
    )
    ...
    return train_acc, valid_acc, test_acc, valid_loss, out, top20_acc, bottom80_acc

@torch.no_grad()
def evaluate_large(model, dataset, split_idx, eval_func, criterion, args, degrees, threshold, device="cpu", result=None):
    if result is not None:
        out = result
    else:
        model.eval()
    ...
    top20_acc = eval_func(
        dataset.label[top20_indices], out[top20_indices]
    ) * top20_indices.shape[0] / split_idx['test'].shape[0]
    bottom80_acc = eval_func(
        dataset.label[bottom80_indices], out[bottom80_indices]
    ) * bottom80_indices.shape[0] / split_idx['test'].shape[0]
    # top_acc = test_acc[]
    if args.dataset in ('yelp-chi', 'deezer-europe', 'twitch-e', 'fb100', 'ogbn-proteins'):
        if dataset.label.shape[1] == 1:
            true_label = F.one_hot(dataset.label, dataset.label.max() + 1).squeeze(1)
